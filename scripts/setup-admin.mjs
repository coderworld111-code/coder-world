...
const email = (clean(process.env.ADMIN_EMAIL) || "coderworld111@gmail.com").toLowerCase();
...
await db.collection("users").doc(user.uid).set({ uid: user.uid, email: user.email || email, name: user.displayName || "CODER WORLD Admin", role: "SUPER_ADMIN", status: "ACTIVE", updatedAt: new Date() }, { merge: true });
console.log(`Admin ready: ${user.email || email} (${user.uid})`);
