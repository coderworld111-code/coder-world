...
## Admin
- Email: set with `NEXT_PUBLIC_COMPANY_EMAIL`
...
## Communication
Contact forms write to Firestore `inquiries`; quote forms write to `leads`; both create an admin `notifications` record targeting the configured `ADMIN_EMAIL`. Configure Firebase Trigger Email (or another server-side Firebase-compatible mail workflow) for actual email delivery.
